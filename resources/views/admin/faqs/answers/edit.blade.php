<x-admin-layout>
    <div class="flex-grow w-full p-5">
        ...
        <div class="overflow-x-auto bg-white rounded-lg mt-5">
            ...
            <form action="{{ route('admin.answers.update', $answer) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-4 px-3">
                                ...
                                <select name="question_id" id="question_id"
                                    ...
                                    @foreach ($questions as $question)
                                        <option value="{{ $question->id }}"
                                            {{ old('question_id', $answer->question_id) == $question->id ? 'selected' : '' }}>
                                            {{ $question->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-span-1">
                            <label class="form-control w-[95%]">
                                ...
                                <select name="status"
                                    ...
                                    <option value="1"
                                        {{ old('status', $answer->status) == 1 ? 'selected' : '' }}>
                                        Phê duyệt</option>
                                    <option value="2"
                                        {{ old('status', $answer->status) == 2 ? 'selected' : '' }}>
                    ...
                    <label class="form-control w-full">
                        ...
                        <input type="text" name="responder" placeholder="Nhập vào"
                            value="{{ old('responder', $answer->responder) }}"
                    ...
                    <label class="form-control w-full">
                        ...
                        <textarea name="answer" id="content"
                            ...
                            rows="1">{{ old('answer', $answer->answer) }}</textarea>
                        @error('answer')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                    </label>
                </div>
                <div class="flex gap-4 justify-center pb-3 mt-3">

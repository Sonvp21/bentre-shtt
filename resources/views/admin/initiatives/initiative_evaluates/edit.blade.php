    ...
    <div class="flex-grow w-full p-5">
        <div class="breadcrumbs text-sm">
                ...
                <ul>
                    ...
                    <li><a class="text-teal-600">Chỉnh sửa</a></li> <!-- Đổi từ "Thêm mới" sang "Chỉnh sửa" -->
                </ul>
        </div>
        <x-admin.alerts.success />
        <div class="overflow-x-auto bg-white rounded-lg mt-5">

                <form action="{{ route('admin.initiative_evaluates.update', $initiativeEvaluate) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') <!-- Sử dụng method PUT để gửi request cập nhật -->
                                ...
                                <select name="initiative_dossier_id" id="initiative_dossier_id"
                                    ...
                                    @foreach ($initiativeDossier as $dossier)
                                        <option value="{{ $dossier->id }}"
                                            {{ old('initiative_dossier_id', $initiativeEvaluate->initiative_dossier_id) == $dossier->id ? 'selected' : '' }}>
                                            {{ $dossier->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <label class="form-control w-[95%]">
                                ...
                                <select name="submission_status"
                                    ...
                                    <option value="1" {{ old('submission_status', $initiativeEvaluate->submission_status) == '1' ? 'selected' : '' }}>Đang
                                        chờ xử lý</option>
                                    <option value="2" {{ old('submission_status', $initiativeEvaluate->submission_status) == '2' ? 'selected' : '' }}>Đang
                                        được xem xét</option>
                                    <option value="3" {{ old('submission_status', $initiativeEvaluate->submission_status) == '3' ? 'selected' : '' }}>Đã
                                        chấm</option>
                                    <option value="4" {{ old('submission_status', $initiativeEvaluate->submission_status) == '4' ? 'selected' : '' }}>Bị
                                        từ chối</option>
                                </select>
                    ...
                    <div class="flex gap-4 justify-center pb-3">
                        ...
                        <button type="submit" class="btn btn-outline btn-accent !min-h-9 h-9 mx-4">Lưu</button> <!-- Đổi từ "Thêm" sang "Cập nhật" -->
                    </div>
                </form>

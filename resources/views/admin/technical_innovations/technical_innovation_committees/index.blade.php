        ...
        <div class="breadcrumbs text-sm">
            <div class="flex ml-auto">
                <a class="btn btn-outline btn-accent !min-h-9 h-9 ml-auto"
                    href="{{ route('admin.technical_innovation_committees.create') }}">
                            ...
                            @foreach ($technicalInnovationCommittees as $index => $technicalInnovationCommittee)
                                <tr>
                                    ...
                                    <td class="flex justify-around">
                                        <a href="{{ route('admin.technical_innovation_committees.edit', $technicalInnovationCommittee) }}"
                                            type="button"><i class="text-yellow-600 fa fa-edit"></i></a>
                                        <form id="delete-form-{{ $technicalInnovationCommittee->id }}"
                                            action="{{ route('admin.technical_innovation_committees.destroy', $technicalInnovationCommittee) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
